       ...
       return new Promise((resolve, reject) => {
         ...
         if(config.activationUrls.indexOf(activationUrl) < 0)
         {
          ...
          axios.post('/Users', userBody).then(response => {
            ...
            emailClient.sendMail({to: user.email, from: config.activationEmailSender, subject: config.activationEmailSubject,
              html: config.activationEmailBody.replace(/\{\{activationUrl\}\}/g,activationUrl)},
