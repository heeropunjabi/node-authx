...
module.exports = function (config, AuthX, services) {
  ...
  router.post('/', (req, res, next) => {
    ...
    services.Registration.createUser(req.body.user, req.body.activationUrl)
      .then(userResponse => {
        config.userEvents.emit('user-signup', { uuid: userResponse.id, activationUrl: userResponse.activationUrl });
        delete userResponse.activationUrl;  //remove the acitivation url as part of the response, security purpose !!
        res.status(201).json({
          success: true,
          data: userResponse
        })
