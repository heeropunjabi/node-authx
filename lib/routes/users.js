...
module.exports = function (config, AuthX, services, eventEmitter) {
  ...
  router.patch('/:id', AuthX.authorize, (req, res, next) => {
    ...
    services.Crud.modifyUser(req.params.id, req.body.user, req.query)
      .then(userResponse => {
        if (userResponse.entitlements) {
          const entitlements = userResponse.entitlements;
          const entitlementIndex = entitlements.indexOf(
            entitlements.filter(entitlement => entitlement.type === 'auth-preference')[0]
          );
          const authPreference = entitlements[entitlementIndex].value;
          if (authPreference === 'true') {
            eventEmitter.emit('user-2fa-enabled', { uuid: userResponse.id });
          } else {
            eventEmitter.emit('user-2fa-disabled', { uuid: userResponse.id });
          }
