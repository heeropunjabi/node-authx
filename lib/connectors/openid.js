 ...
 */
module.exports = function (openIdConfiguration, scimConfiguration = {}, cb) {
  ...
  return {
    /**
     * this callback is registered considering the xblog npm module.
     * we need this callback to be called on every authroize function called.
     */
    cb: cb,
    /**
    ...
    authorize: function (
      ...
      openIdClient
        ...
        .then(introspectionResponse => {
          if (introspectionResponse.scopes.indexOf(scope) < 0) {
            ...
            openIdClient.userinfo(token).then((userInfo) => {
              ...
              if (this.cb) {
                this.cb(null, request, response, next, response.locals.authx.uuid);
              }
              next();
            });
          }
        ...
        .catch(error => {
          if (this.cb) {
            this.cb(error, request, response, next);
          }
